---
title: "..."
subtitle: "Combined cNFs + HLS and HS from ctrl, Tom+"
author: "Pernelle"
...
```{r out_dir}
save_name = "combined_cNFs_HS_Tom_cells"
out_dir = "."
```
...
```{r project_names_oi}
project_names_oi = c("2023_17", "2023_18","2023_21","2023_27","2023_32","2023_34","2023_36","2023_40","2023_48","2023_44","2023_46")
```
...
column_of_interest = c("orig.ident", "nCount_RNA", "nFeature_RNA",
                       ...
                       "age", "genotype",
                       "condition", "sample_identifier", "sample_type","percent.st","cell_type")
...
```{r harmony not working, fig.width = 8, fig.height = 5, time_it = TRUE, eval=FALSE, echo=FALSE}
# `%||%` = function(lhs, rhs) {
#   if (!is.null(x = lhs)) {
#     return(lhs)
#   } else {
...
```{r table_cell_type, fig.width = 11, fig.height = 5, class.source = "fold-hide"}
...
custom_theme2 = gridExtra::ttheme_minimal(base_colour = c("black", "black", "black"),
                                          core = list(bg_params = list(fill = c("white", "white", "white"),
                                                                       col = "black")),
                                          colhead = list(fg_params = list(col = "black")))
gb2 = data.frame(cell_family = c("Glial cells", "Immune cells", "Fibroblasts", "Immune cells", "Vascular cells","Melanocytes","Keratinocytes")) %>%
  gridExtra::tableGrob(.,
                       row = NULL,
                       cols = c("Cell family"),
                       theme = custom_theme2)
...
#gb$layout[gb$layout$t != 1 & gb$layout$l == 1, c("t")] = c(2, 9, 15, 16, 18, 19, 20, 2, 9, 15, 16, 18, 19)
#gb$layout[gb$layout$b != 1  & gb$layout$l == 1, c("b")] = c(2, 9, 15, 16, 18, 19, 20, 2, 9, 15, 16, 18, 19)
...
# cluster.markers <- Seurat::FindMarkers(sobj, ident.1 = c(8), ident.2 = c(2,6,11,0,3), min.pct = 0.25)
# head(cluster.markers, n = 30)
# ```
#cluster.markers%>%filter(avg_logFC>1&p_val_adj<0.01)%>%rownames()%>%aquarius::print_to_copy_paste()

```{r proportion_cluster, eval=FALSE, echo=FALSE}
proportion_clusters = aquarius::plot_barplot(df = table(sobj$seurat_clusters,
                                  sobj$condition) %>%
                         as.data.frame.table() %>%
                         `colnames<-`(c("Sample", "Cell Type", "Number")),
                       x = "Sample", y = "Number", fill = "Cell Type",
...
## Differential gene expression between dissociation methods

# ```{r markers_dissociation, eval=FALSE, echo=FALSE}
# subsobj = subset(sobj, cluster_type == "Non-myelinating schwann cells")
# Seurat::Idents(subsobj) = subsobj$dissociation
# mark =  Seurat::FindMarkers(subsobj, ident.1="3hLib", ident.2="18hLib")
# head(mark, n = 50)
# ```
