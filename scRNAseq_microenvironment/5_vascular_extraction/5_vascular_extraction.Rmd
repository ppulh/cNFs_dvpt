---
title: "Vascular cells extraction - microenvironment (cNFs + HLS + HS)"
subtitle: "Zoom in vascular cells"
author: "Pernelle"
...
sobj_list = lapply(sobj_list, FUN = function(one_sobj) {
  ...
  cluster_of_interest = score_by_clusters %>%
    ...
    as.character()
  one_sobj$is_VC = ifelse(one_sobj$seurat_clusters %in% cluster_of_interest,
                                   yes = TRUE, no = FALSE)
...
sobj_list = lapply(sobj_list, FUN = function(one_sobj) {
  if (sum(one_sobj$is_VC) > 0) {
    one_sobj = subset(one_sobj, is_VC == TRUE)
  } else {
    one_sobj = NA
  }
  return(one_sobj)
